...
const safeBreakdown = FIXED_KEYS.map((label) => {
  const found = view?.breakdown?.find(
    (d: any) => d.type === label || d.type?.en === label 
  );
  return {
    type: label,
    value: found ? Math.round(found.value ?? 0) : 0, 
  };
});
                        ...
                        {blocks.map((block, idx) => {
                          ...
                          return (
                            <div key={`q-${qIndex}`}>
                              ...
                              {firstLine && (
                                <blockquote
                                  className="italic text-muted-foreground mt-1 font-inherit"
                              ...
                              {restLines.length > 0 && (
                                <div
                                  className="whitespace-pre-line mt-2 font-inherit"
                                  dangerouslySetInnerHTML={{
        ...
        <Card>
          ...
          <CardContent>
            {Array.isArray(view?.external) && view.external.length > 0 ? (
              <div
                className="whitespace-pre-line font-inherit"
                dangerouslySetInnerHTML={{
