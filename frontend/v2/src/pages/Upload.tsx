...
const Upload = () => {
  ...
  const [customRules, setCustomRules] = useState(false); // off=legacy, on=rules_llm
  ...
  /** pick current UI language to send to backend as overrided_language */
  const currentLang =
    (typeof window !== "undefined" && (localStorage.getItem("i18nextLng") || i18n.language)) ||
    "en";

  /** user selects files by click/drag */
  ...
  /** helper: convert toggle to backend rules_mode */
  const getRulesMode = () => (customRules ? "rules_llm" as const : "legacy" as const);
  ...
  const handleStart = async () => {
    ...
    // IMPORTANT: send rules mode and language to backend
    formData.append("rules_mode", getRulesMode());
    formData.append("overrided_language", currentLang);
    ...
    try {
      ...
      // Fake progress to give user feedback while backend works
      ...
      // If backend detects duplicate -> show dialog
      ...
      if (e?.message) {
        if (e.message.includes("timeout")) errorMessage = t("upload.errors.timeout");
        else if (e.message.includes("ECONNABORTED")) errorMessage = t("upload.errors.connectionFailed");
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="grid gap-6">
              {/* Priority select (UI only, not used by backend now) */}
              ...
              {/* Rules mode toggle → legacy / rules_llm */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
                ...
                <div className="flex items-center gap-3">
                  <Switch
                    ...
                    aria-label="Enable rules + LLM scoring"
                  />
                  <span className="text-sm text-muted-foreground">
                    {/* UI hint: ON → rules_llm, OFF → legacy */}
                    {t("upload.customRulesDesc")}
                  </span>
                </div>
              </div>

              {/* Optional note (not sent to backend by default) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
              ...
              {uploading && (
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground text-center">{t("upload.analysisTimeWarning")}</p>
                </div>
              )}
      ...
      <AlertDialog open={showDuplicateDialog} onOpenChange={setShowDuplicateDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("upload.duplicateDialog.description")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            {/* Show old analysis */}
            <AlertDialogCancel
            ...
            <AlertDialogAction
              onClick={async () => {
                try {
                  ...
                  const formData = new FormData();
                  formData.append("file", files![0]);
                  formData.append("session_id", `s_${Date.now()}`); // new session id
                  formData.append("force_new", "true");

                  // IMPORTANT: keep same mode & language for re-analysis
                  formData.append("rules_mode", getRulesMode());
                  formData.append("overrided_language", currentLang);
